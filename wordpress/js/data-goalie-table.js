document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("/wp-content/uploads/tsa-data/goalies/wordpress_goalie_refresh_meta.json")
    ...
    .then(meta => {
      ...
      lastUpdatedBox.textContent = "Goalie data updated: " + formatted;
    })
    .catch(() => {
      lastUpdatedBox.textContent = "Goalie data updated: Unavailable";
    });
  ...
  let currentEndpoint = "/wp-json/tsa/v1/goalie-bios";
  ...
  fetch("/wp-json/tsa/v1/goalie-team-options")
    .then(response => response.json())
    .then(data => {
    ...
    .catch(() => {
      console.warn("Unable to load dynamic goalie team lists.");
    });
  ...
  function getMetaEndpoint() {
    return `/wp-json/tsa/v1/goalie-${currentDataset}-meta`;
  }

  function getCsvEndpoint(full = false) {
    const base = `/wp-json/tsa/v1/goalie-${currentDataset}-csv`;

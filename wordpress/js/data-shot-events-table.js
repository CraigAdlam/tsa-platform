document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("/wp-content/uploads/tsa-data/shot_events/wordpress_shot_events_refresh_meta.json")
    ...
    .then(meta => {
      ...
      lastUpdatedBox.textContent = "Shot events data updated: " + formatted;
    })
    .catch(() => {
      lastUpdatedBox.textContent = "Shot events data updated: Unavailable";
    });
  ...
  function getMetaEndpoint() {
    return `/wp-json/tsa/v1/shot-events-${currentDataset}-meta`;
  }

  function getCsvEndpoint(full = false) {
    const base = `/wp-json/tsa/v1/shot-events-${currentDataset}-csv`;
  ...
  document.getElementById("tsa-shot-region-filter").addEventListener("change", refreshTable);
  document.getElementById("tsa-shot-type-filter").addEventListener("change", refreshTable);
